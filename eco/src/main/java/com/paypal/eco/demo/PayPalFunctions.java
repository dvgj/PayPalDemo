...
public class PayPalFunctions {
	...
	public PayPalFunctions() {// lhuynh - Actions to be Done on init of this
		...
		gv_APIUserName = "jamesdavid-facilitator_api1.fastmail.fm";
		gv_APIPassword = "1407287604";
		gv_APISignature = "AZ0JgFjRjwUW1SVrdAgh8B9kbzdoAfRpp7zQl98oRaU7tAVxuhZN83v7";
	...
	public HashMap SetExpressCheckout(String paymentAmount, String returnURL,
		...
		nvpstr = nvpstr.concat("&PAYMENTREQUEST_0_ITEMAMT=" + paymentAmount
		...
		nvpstr = nvpstr
				.concat("&PAYMENTREQUEST_0_SHIPTOZIP="
						+ shipToZip
						+ "&REQCONFIRMSHIPPING=0&NOSHIPPING=0&PAYMENTREQUEST_0_SHIPTOPHONENUM"
						+ phoneNum);
	 ...
	 * Output: Returns a HashMap object containing the response from the server.
	 *********************************************************************************/
	public HashMap DoExpressCheckoutPayment(String token, String payerID,
	...
	public HashMap httpcall(String methodName, String nvpStr) {
		...
		try {
			URL postURL = new URL(gv_APIEndpoint + "?" + encodedData);
			...
			if (rc != -1) {
				...
				// added code to show the actual text response from paypal.
				HttpServletRequest req = (HttpServletRequest) ExpressCheckoutServlet.td
						.get();
				if (req != null) {
					req.setAttribute("paypalResponse", respText);
				}
